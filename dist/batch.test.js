 ...
 */
function one_then_ended_source(value) {
    return {
        // oxlint-disable-next-line eslint/require-yield
        *[Symbol.iterator]() {
            let sent = false;
 ...
 */
function ends_later_source(value, delay) {
    return {
        // oxlint-disable-next-line eslint/require-yield
        *[Symbol.iterator]() {
            let pulls = 0;
 ...
 */
function* take_batches(stream, n) {
    const out = [];
...
describe("maxSize batching", () => {
    ...
    test("emits as soon as maxSize is reached, before maxTime could fire", async () => {
        ...
        const batches = await run(function* () {
            ...
            const consumer = yield* spawn(() => take_batches(batch({ maxSize: 3, maxTime: 10_000 })(src.stream), 2));
    ...
    test("with both options, a partial batch is emitted when maxTime fires first", async () => {
        const src = make_source();
        const batches = await run(function* () {
            const consumer = yield* spawn(() => take_batches(batch({ maxTime: 60, maxSize: 10 })(src.stream), 1));
            src.push("A");
            yield* sleep(20);
    ...
    test("maxSize of 1 emits singleton batches immediately, without waiting on the window", async () => {
        ...
        const src = make_source();
        const batches = await run(function* () {
            const consumer = yield* spawn(() => take_batches(batch({ maxSize: 1, maxTime: 60_000 })(src.stream), 3));
            for (const n of [1, 2, 3]) {
                src.push(n);
            }
            return yield* consumer;
...
describe("Stream<T, never> contract", () => {
    test("throws if the source ends immediately", async () => {
        await expect(run(() => take_batches(batch({ maxTime: 50 })(ended_source()), 1))).rejects.toThrow(/ended unexpectedly/);
    });
    test("throws if the source ends after emitting part of a batch", async () => {
        await expect(run(() => take_batches(batch({ maxTime: 50 })(one_then_ended_source(1)), 1))).rejects.toThrow(/ended unexpectedly/);
    });
    test("throws when the contract violation arrives on a carried pull", async () => {
        ...
        await expect(run(() => take_batches(batch({ maxTime: 40 })(ends_later_source(1, 100)), 2))).rejects.toThrow(/ended unexpectedly/);
    });
});
// ----------------------------------------------------------------------------
...
describe("halt safety of next()", () => {
    ...
    test("halting next() during the first, deadline-free pull leaves the source pull pending", async () => {
        ...
        const src = halt_counting_source();
        await run(function* () {
            const subscription = yield* batch({ maxTime: 50 })(src.stream);
            const outcome = yield* timebox(20, () => subscription.next());
