 ...
 */
function assert_valid_delay(ms) {
    if (typeof ms !== "number" ||
        !Number.isFinite(ms) ||
        ms < MIN_DELAY_MS ||
        ms > MAX_DELAY_MS) {
        throw new Error(`ticker delay must be a number of milliseconds ` +
 ...
 */
function notify_waiters(waiters) {
 ...
 */
function wait_for_delay_change(waiters, get_revision, observed_revision) {
 ...
 */
export function adjustable_interval(delay) {
