 ...
 */
function make_tracked() {
    let sig = createSignal();
    let torn = false;
    let stream = resource(function* (provide) {
        let sub = yield* sig;
        try {
            yield* provide(sub);
        }
 ...
 */
function throwing_stream(values, err, delay = 0) {
    return {
        // oxlint-disable-next-line eslint/require-yield
        *[Symbol.iterator]() {
            let i = 0;
 ...
 */
function boom_on_subscribe(err) {
    return {
        // oxlint-disable-next-line eslint/require-yield
        *[Symbol.iterator]() {
            throw err;
