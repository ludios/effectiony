...
import { ensure_durable_nats_consumer, ensure_nats_stream, get_nats_consumer, nats_jetstream, nats_jetstream_manager, use_nats_connection, use_nats_consumer_messages, use_ordered_nats_consumer, } from "./nats.js";
/**
...
async function start_nats_server() {
    ...
    const child = spawn_child("nats-server", ["-a", "127.0.0.1", "-p", "-1", "-js", "-sd", join(directory, "jetstream"), "--ports_file_dir", directory], { stdio: ["ignore", "ignore", "pipe"] });
    let stderr = "";
    ...
    while (Date.now() < deadline && child.exitCode === null && child.signalCode === null) {
        try {
            const ports = JSON.parse(await readFile(ports_path, "utf8"));
            url = ports.nats?.[0];
            if (url) {
                break;
            }
 ...
 */
function* create_empty_consumer(connection, stream_name) {
    const consumer_name = "worker";
    const manager = yield* nats_jetstream_manager(connection);
    yield* ensure_nats_stream(manager, stream_name, { subjects: [`${stream_name}.>`] });
    yield* ensure_durable_nats_consumer(manager, stream_name, consumer_name, {
